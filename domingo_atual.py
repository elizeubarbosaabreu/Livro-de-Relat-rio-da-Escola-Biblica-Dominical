...
print("Gerando relatório da Escola Bíblica Dominical...")
# Aguarda 2 segundos antes de continuar
sleep(2)
# Define a localidade para formatação de data       
...
def gerar_pdf():
    ...
    for classe in classes:
        ...
        if perc >= 80:
            pdf.set_fill_color(144, 238, 144)  # verde-limão
            fill = True
        elif perc < 60:
            pdf.set_fill_color(255, 99, 71)  # vermelho (Tomato)
            fill = True
        else:
            pdf.set_fill_color(255, 255, 153)  # amarelo-claro
            fill = True
    ...
    pdf.set_font("Arial", '', 10)
    for campo in ["Matriculados", "Ausentes", "Presentes", "Visitantes", "Total", "Bíblias", "Revistas", "Ofertas", "% de Presença"]:
